    ...
    def cancel(self, task_id: str) -> bool:
        ...
        if on_cancel:
            try:
                ...
                print(f"[TaskQueue] on_cancel callback error: {exc}")
        with self._condition:
            self._condition.notify_all()
        return True
        ...
        try:
            ...
            if not task.cancel_flag.is_set():
                ...
                if self._awareness and task.kind != "research":
                    try:
                        ...
                        print(f"[TaskQueue] awareness completion update failed: {e}")

                if task.speak:
                    try:
                        task.speak(completion_msg)
                    except Exception as e:
                        print(f"[TaskQueue] speak completion notification failed: {e}")
            ...
            if task.kind != "research":
                try:
                    ...
                    print(f"[TaskQueue] task history save failed: {e}")
            ...
            if self._awareness and task.kind != "research":
                try:
                    ...
                    print(f"[TaskQueue] awareness failure update failed: {ae}")

            if task.speak and not cancelled:
                try:
                    task.speak(failure_msg)
                except Exception as se:
                    print(f"[TaskQueue] speak failure notification failed: {se}")
            ...
            if task.kind != "research":
                try:
                    ...
                    print(f"[TaskQueue] task history save failed: {he}")
