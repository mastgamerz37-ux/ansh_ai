...
def _get_api_key() -> str:
    ...
    key = get_gemini_key() or os.environ.get("GEMINI_API_KEY", "").strip()
    if key:
        return key
...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    ...
    if speak:
        speak("Executing specialized task code.")
...
def _detect_language(text: str) -> str:
    ...
    try:
        response = model.generate_content(
            ...
            f"Reply with ONLY the language name in English (e.g. Hindi, English, Spanish, Turkish, French).\n\n"
            f"Text: {text[:200]}"
        )
...
The following report was generated from live research results gathered by ANSH. It consolidates the most relevant findings into a single saved document for later review.
...
- Information gathered and verified from online sources.
- Stored for direct user reference.
...
def _translate_to_goal_language(content: str, goal: str) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"IMPORTANT:\n"
            f"- Translate naturally, preserving terms and numbers\n"
    ...
    def _summarize(self, goal: str, completed_steps: list, speak: Callable | None, step_results: dict | None = None) -> str:
        ...
        try:
            ...
            prompt    = (
                ...
                "Do not claim uncertainty if the tool result confirms the file was saved. Be direct and concise."
            )
            response = model.generate_content(prompt)
