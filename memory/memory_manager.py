...
from memory.config_manager import get_app_data_dir

BASE_DIR         = get_base_dir()
LOCAL_MEMORY     = BASE_DIR / "memory" / "long_term.json"
APPDATA_DIR      = get_app_data_dir()
MEMORY_PATH      = APPDATA_DIR / "long_term.json"
_lock            = Lock()
...
def load_memory() -> dict:
    if not MEMORY_PATH.exists() and LOCAL_MEMORY.exists():
        try:
            MEMORY_PATH.parent.mkdir(parents=True, exist_ok=True)
            MEMORY_PATH.write_text(LOCAL_MEMORY.read_text(encoding="utf-8"), encoding="utf-8")
        except Exception:
            pass
