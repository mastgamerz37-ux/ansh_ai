...
class AwarenessEngine:
    def __init__(
        self,
        popup_scheduler: Callable[[str, PopupType], None] | None = None,
        check_interval: float = 5.0,
        insight_cooldown: float = 300.0,
    ):
        self.popup_scheduler = popup_scheduler or (lambda msg, t: None)
    ...
    def start(self) -> None:
        ...
        self._thread.start()
        print("[Awareness] Awareness engine started")

    def stop(self) -> None:
        self._running = False
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=2.0)
        print("[Awareness] Awareness engine stopped")
        ...
        try:
            if sys.platform == "win32":
                ...
                for p in psutil.process_iter(['name']):
                    if p.info['name'] and 'code' in p.info['name'].lower():
                        return True
                return False
        ...
        try:
            if sys.platform == "win32":
                ...
                if win and win.title:
                    return win.title.split(" - ")[-1].strip()
            elif os.name == "posix":
                script = (
...
_global_awareness: AwarenessEngine | None = None
...
def get_awareness_engine() -> AwarenessEngine:
    ...
    with _global_lock:
        if _global_awareness is None:
            _global_awareness = AwarenessEngine()
            _global_awareness.start()
