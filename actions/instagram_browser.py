...
INSTAGRAM_PROFILE = Path.home() / ".ansh_profiles" / "instagram"
    ...
    def _worker(self) -> None:
        ...
        while True:
            ...
            try:
                if self._init_error:
                    raise RuntimeError(
                        "Playwright is not ready. Install browser support with: python -m playwright install chromium"
...
"""
    ...
    def _login_message(self) -> str:
        return "Instagram login required. I opened the ANSH Instagram browser; log in there, then try again."
  ...
  const findScrollable = () => {
    ...
    const candidates = Array.from(document.querySelectorAll('div, section, main'))
      ...
      .sort((a, b) => {
        ...
        const bScore = (headingBottom && br.top <= headingBottom + 40 ? 10000 : 0) + (br.scrollHeight - b.clientHeight);
...
"""
    ...
    def _resolve_chat(self, page, receiver: str, message: str) -> tuple[dict | None, str | None]:
        ...
        if not all_rows:
            return None, "I could not read your Instagram DM list. Open Instagram in browser and log in."
...
"""
    ...
    def _prepare_draft(self, receiver: str, message: str) -> str:
        ...
        if recipient:
            ...
            if not recipient:
                return "Open an Instagram DM in the browser, or provide a recipient name."

        if not self._fill_composer(page, message):
            return "I could not type into the Instagram composer."
    ...
    def _send_draft(self) -> str:
        ...
        if not self._pending:
            return "Could not find a pending Instagram draft."
        self._dismiss_interruptions(page)

        if not self._send_composer(page):
            return "Instagram did not confirm send."
