...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    # Yapısal geri dönüş
    desc_l = desc.lower()
    if any(w in desc_l for w in ("screen_debug", "screen debug", "debug screen")):
        return "screen_debug"
    if any(w in desc_l for w in ("optimize", "refactor", "speed up", "clean up", "performance")):
        return "optimize"
    if any(w in desc_l for w in ("build", "iterate", "create and test")):
        return "build"
    if any(w in desc_l for w in ("run", "execute", "start script")):
        return "run"
    if file_exists:
        return "edit" if desc else "explain"
    if code:
        if any(w in desc_l for w in ("edit", "change", "modify", "update", "fix")):
            return "edit"
        return "explain"
    if any(w in desc_l for w in ("explain", "what does", "describe", "review")):
        return "explain"
    return "write"
