"""Create polished, editable PowerPoint presentations for ANSH/JARVIS users."""
...
DISPLAY_FONT = "Segoe UI" if platform.system() == "Windows" else "Avenir Next"
BODY_FONT = "Segoe UI" if platform.system() == "Windows" else "Avenir Next"
DATA_FONT = "Consolas" if platform.system() == "Windows" else "Menlo"
...
def _api_key() -> str:
    ...
    key = str(get_gemini_key() or os.environ.get("GEMINI_API_KEY") or "").strip()
    if not key:
    ...
    for index in range(count):
        ...
        if index == count - 1:
            slides.append({
                ...
                "subtitle": evidence_at(index, f"Essential conclusions and direction for {topic}."),
            })
            continue
        ...
        slide = {
            ...
            "left_items": ["What is known", "Key parameters"],
            "right_items": ["Strategic goals", "Immediate next steps"],
        }
        slides.append(slide)
    return {
        "title": topic,
...
def _render_presentation(
    ...
    prs.core_properties.subject = "Editable presentation"
    blank = next(
    ...
    *,
    ...
    try:
        ...
        if _presentation_quota_cooldown_active() and mode in {"create", "redesign"}:
            ...
            warnings.append(
                "Gemini presentation planning is in a short quota cooldown. A minimal local deck was completed."
            )
            qa["quota_fallback"] = {
                "used": True,
            ...
            try:
                ...
                ai_quota_exhausted = True
                warnings.append(
                    "Gemini planning quota was unavailable. A minimal local deck was completed."
                )
                qa["quota_fallback"] = {
                    "used": True,
                    "stage": "planning",
        ...
        if mode != "edit":
            ...
            if preflight_issues:
                warnings.append(
                    "Local presentation preflight notes: " + "; ".join(preflight_issues[:3])
                )
