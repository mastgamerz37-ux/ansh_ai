"""Cancellable, source-grounded deep research for ANSH/JARVIS."""
...
def _duckduckgo_html_search(query: str, max_results: int) -> list[dict]:
    ...
    response = requests.get(
        ...
        params={"q": query},
        headers={"User-Agent": "Mozilla/5.0 ANSH Research"},
        timeout=18,
    )
...
Reviewed {len(sources)} distinct web sources for this question. The synthesis model was unavailable, so this report preserves the retrieved evidence directly instead of inventing a conclusion.
...
def _save_report(result: ResearchResult, destination: str, output_path: str = "") -> Path:
    ...
    if output_path:
        ...
        report_path = Path.home() / "Desktop" / "Research" / f"{_slug(result.question)}-{stamp}.md"
    else:
        report_path = Path.home() / "Documents" / "Research" / f"{_slug(result.question)}-{stamp}.md"
