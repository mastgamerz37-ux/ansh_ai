...
are typed into ANSH's controlled browser and sent only after a later explicit
...
def _load_gmail_credentials():
    ...
    if not payload:
        raise GmailConnectionError(
            ...
            "then tell ANSH: connect Gmail using this file."
        )
    try:
...
def _gmail_connect(credentials_path: str = "") -> str:
    ...
    if path is None:
        return (
            ...
            "download the JSON, then place it in config/ or Downloads and say: connect Gmail."
        )
    try:
        ...
        credentials = flow.run_local_server(
            ...
            access_type="offline",
            prompt="consent",
            success_message="Gmail is connected to ANSH. You can close this browser tab.",
        )
...
async def _assist_gmail_sign_in(page, player=None, timeout_seconds: float = 90.0) -> bool:
    ...
    _gmail_login_progress(player, "Complete Google password or 2FA; compose will resume")
    deadline = asyncio.get_running_loop().time() + timeout_seconds
    while asyncio.get_running_loop().time() < deadline:
...
async def _open_gmail_compose_fields(session, player=None):
    ...
    if signed_out:
        if not await _assist_gmail_sign_in(page, player=player):
            return page, None, (
                "Gmail is waiting for Google sign-in. Complete the password or 2FA step in the browser window, "
                "then ask me to prepare the email again."
            )
        try:
            await page.goto(compose_url, wait_until="domcontentloaded", timeout=45_000)
        except Exception:
    ...
    if subject is None or body is None:
        ...
        return page, None, "Gmail opened, but could not find a new compose window. Try opening Gmail once and signing in."
    ...
    if to_field is None:
        return page, None, "Gmail opened a compose window, but could not locate the recipient field."
...
async def _gmail_web_prepare_once(session, draft: dict[str, str], player=None) -> str:
    ...
    for field_name, selectors in (
        ...
        if locator is None:
            return f"Gmail opened the draft, but could not locate the {field_name} field. Nothing was sent."
...
async def _gmail_web_prepare_async(session, draft: dict[str, str], player=None) -> str:
    ...
    return (
        "Gmail interrupted the compose operation twice, so the incomplete draft was discarded. "
...
async def _gmail_web_send_async(session, draft: dict[str, str]) -> str:
    ...
    if send_button is None:
        return "Could not find Gmail's Send button. Nothing was sent."
...
def _mail_error(result: subprocess.CompletedProcess, operation: str) -> str:
    detail = (result.stderr or result.stdout or "Mail did not respond").strip()
...
def _prepare_email(params: dict, player=None) -> str:
    ...
    return (
        ...
        + ("ANSH opened Gmail and visibly typed this draft. It remains open for your review.\n" if provider == "gmail" else "")
...
def email_control(parameters: dict | None = None, response=None, player=None, session_memory=None) -> str:
    ...
    if action == "disconnect":
        _clear_gmail_credentials()
        return "The local Gmail connection was removed."
    if action == "inbox":
        limit = _bounded_limit(params.get("limit", 10))
        if provider == "gmail":
            return _gmail_list("in:inbox", limit, "Recent Gmail inbox")
        return "Inbox access for this provider is unavailable. Connect Gmail."
    if action == "unread":
        limit = _bounded_limit(params.get("limit", 10))
        if provider == "gmail":
...
def get_pending_email() -> dict | None:
    return _get_pending_email()


def get_email_state() -> dict:
    return {
        "connected": bool(_gmail_account()),
        "account": _gmail_account(),
        "has_pending_draft": bool(_get_pending_email()),
    }
