...
def _basic_key_check(api_key: str) -> ApiKeyValidationResult | None:
    ...
    if key.startswith("sk-ant-"):
        return ApiKeyValidationResult(False, "That is an Anthropic key. A Gemini API key is required.")
...
def _validate_with_gemini(api_key: str, timeout_seconds: float) -> ApiKeyValidationResult:
    try:
        ...
        if "permission" in lowered or "403" in lowered:
            return ApiKeyValidationResult(
                False,
                "Gemini rejected this key's permissions. Create an authorization key in Google AI Studio.",
            )
        return ApiKeyValidationResult(
            False,
            f"Gemini could not verify this key: ({text[:120]})",
        )
