"""Shared Gemini Live model selection for startup audio and active session."""
...
def pick_live_model(client, config_path: Path) -> str:
    ...
    try:
        ...
        print(
            f"[LiveModel] Could not list Gemini models; using configured "
    ...
    print(f"[LiveModel] Auto-selected Live model: {selected}")
    return selected
